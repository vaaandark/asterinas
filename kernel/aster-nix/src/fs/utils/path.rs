...
#[derive(Debug)]
pub struct Path {
    mntnode: Arc<MountNode>,
    dentry: Arc<Dentry>,
    this: Weak<Path>,
}

impl Path {
    pub fn new(mntnode: Arc<MountNode>, dentry: Arc<Dentry>) -> Arc<Self> {
        ...
        let path = match name {
            ...
            name => {
                ...
                match children_dentry {
                    Some(dentry) => Path::new(self.mntnode().clone(), dentry.clone()),
                    None => {
                        let slow_dentry = self.dentry.lookup_slow(name)?;
                        Path::new(self.mntnode().clone(), slow_dentry.clone())
                    }
                }
            }
        };
    ...
    fn effective_name(&self) -> String {
        ...
        if !self.mntnode.parent().is_none() & !self.mntnode.mountpoint_dentry().is_none() {
            let parent_path = Path::new(
                self.mntnode.parent().unwrap().upgrade().unwrap().clone(),
                self.mntnode.mountpoint_dentry().unwrap().clone(),
            );
            return parent_path.effective_name();
        } else {
            return self.dentry.name();
        }
    }
    ...
    fn effective_parent(&self) -> Option<Arc<Self>> {
        ...
        if !self.mntnode.parent().is_none() & !self.mntnode.mountpoint_dentry().is_none() {
            ...
            return parent_path.effective_parent();
        } else {
            return None;
        }
    }
    ...
    fn overlaid_path(&self) -> Arc<Self> {
        ...
        match self.mntnode.get(&self.dentry) {
            Some(child_mount) => {
    ...
    pub fn mount(&self, fs: Arc<dyn FileSystem>) -> Result<Arc<MountNode>> {
        ...
        if self.effective_parent().is_none() {
            return Ok(self.mntnode().clone());
            // return_errno_with_message!(Errno::EINVAL, "can not mount on root");
        }
        let child_mount = self.mntnode().mount(fs, &self.this())?;
    ...
    pub fn link(&self, old: &Arc<Self>, name: &str) -> Result<()> {
        ...
        if !Arc::ptr_eq(&old.mntnode(), &self.mntnode()) {
        ...
        let dentry = self.dentry.new_in_path(old_inode.clone(), name);
    ...
    pub fn mntnode(&self) -> &Arc<MountNode> {
        &self.mntnode
    }
